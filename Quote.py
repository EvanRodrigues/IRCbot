...
class Quote():
    ...
    def print_quote(self, socket, irc, index, username):
        ...
        if totalQuotes < index or index < 1:
            send_message(socket, irc, "Please choose an integer between 1 and " +
                         str(totalQuotes) + " " + username)
            self.active = False
            return
    ...
    def run(self, socket, irc, index, username):
        QuoteThread = threading.Thread(
            target=self.print_quote, args=(socket, irc, index, username))
        QuoteThread.start()
