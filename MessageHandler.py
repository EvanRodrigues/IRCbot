...
def get_bits(username, message):
	return 1

def removeTags(line):
	parts = line.split(":")
	total_parts = len(parts)
	message = parts[total_parts - 1]
...
class MessageHandler:
	def __init__(self):
		self.game = ""
		self.startTime = None
		self.id = {'Client-ID': CLIENT_ID}
		self.q = "https://api.twitch.tv/helix/streams?user_login=doopian"


	def get_game(self):
		while True:
			r = requests.get(url = self.q, headers = self.id)
			data = r.json()
			r.close()
			...
			self.game = get_game_name(data['data'][0]['game_id'], self.id)
			self.startTime = data['data'][0]['started_at']
			time.sleep(60)


	#TODO: Set a timer for !wr
	def message_handler(self, irc, s, line, quote):
		username = get_tag("display-name", line)
		bits = get_tag("bits", line)


		message = removeTags(line)
		...
		#random commands
			...
			send_message(s, irc, get_world_record(self.game))
